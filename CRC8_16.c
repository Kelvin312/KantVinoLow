...
flash unsigned char auchCRC8[256] = {
...
{
    rx_crc_hi = 0xFF; // high byte of CRC initialized  
    rx_crc_lo = 0xFF; // low byte of CRC initialized
}

inline void CRC16_Add(char value)
{
    unsigned char i = rx_crc_hi ^ value;
    rx_crc_hi = rx_crc_lo ^ auchCRCHi[i];
    rx_crc_lo =          auchCRCLo[i];
}

inline char CRC16_Check()
{
    return rx_crc_hi | rx_crc_lo;
}
